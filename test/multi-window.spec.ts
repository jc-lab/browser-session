...
describe('multiWindow', () => {
  it('sessionPropagationTest', async () => {
    ...
    expect(windows[0].localStorage.getItem('bbbb')).not.toBeNull();
    expect(windows[1].localStorage.getItem('bbbb')).not.toBeNull();
    expect(windows[0].sessionStorage.getItem(BrowserSession.STORAGE_SECRET_KEY_NAME)).not.toBeNull();
    expect(windows[1].sessionStorage.getItem(BrowserSession.STORAGE_SECRET_KEY_NAME)).not.toBeNull();
    expect(windows[0].sessionStorage.getItem(BrowserSession.STORAGE_SECRET_KEY_NAME))
        .toEqual(windows[1].sessionStorage.getItem(BrowserSession.STORAGE_SECRET_KEY_NAME));
  });
